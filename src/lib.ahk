...
TreeViewEx_HandlerContextMenu(tvex, wParam, lParam, *) {
    if tvex.Enabled {
        ...
        ; If the context menu was activated by right-click
            ...
            pt := TreeViewEx_Point(X, Y)
            pt.ScreenToClient(tvex.Hwnd, true)
 ...
 */
TreeViewEx_RectMoveAdjacent(Subject, Target?, ContainerRect?, Dimension := 'X', Prefer := '', Padding := 0, InsufficientSpaceAction := 0) {
    Result := 0
    if IsSet(Target) {
        tarL := Target.L
        tarT := Target.T
