...
TreeViewEx_SubclassProc(Hwnd, uMsg, wParam, lParam, uidSubclass, dwRefData) {
    ; When using the debugger in VSCode, `OnExit` callbacks were not executing if I stopped
    ; the script by clicking the end/restart button in the debugger toolbar, resulting in
    ; nothing disabling the window subclass, resulting in a lot of errors because notifications
    ; were being processed after AutoHotkey deleted the "Call" method. This condition was
    ; added for that circumstance.
    if HasMethod(Object, 'Call') {
        switch uMsg {
            case WM_NOTIFY:
                notifyManager := ObjFromPtrAddRef(dwRefData)
                hdr := TvNmHdr.FromPtr(lParam)
                if collectionControl := notifyManager.Collection.Get(hdr.hwndFrom) {
                    if list := collectionControl.Get(hdr.code_int) {
                        struct := hdr.Cast()
                        for callback in list {
                            return callback(collectionControl.Ctrl, struct)
                        }
                    }
                }
        }
...
TreeViewEx_HandlerKeyDown(Ctrl, _tvKeyDown) {
    return Ctrl.__HandlerKeyDown.Call(_tvKeyDown.wVKey, _tvKeyDown)
}
...
TreeViewEx_SetConstants() {
    ...
    TreeViewEx.LibToken := LibraryManager(Map(
      ...
      , 'msvcrt', [ 'memmove' ]
      , 'user32', [ 'CreateWindowExW', 'DestroyWindow', 'GetDpiForWindow' ]
