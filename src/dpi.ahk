/**
    This file is not included in VENV.ahk.
    #include it separately and call {@link TreeViewEx_SetDpiConstants}.
*/

TreeViewEx_SetDpiConstants(force := false) {
    global
    ...
    tvex_flag_dpiconstants_set := true

    TreeViewEx.DpiLibToken := LibraryManager(Map(
        'user32'
      , [
            'GetDpiForWindow'
 ...
 * Identical to TreeViewEx_GetAwarenessFromDpiAwarenessContext(TreeViewEx_GetThreadDpiAwarenessContext());
...
/**
 * @description - This method will return the latest DPI_AWARENESS_CONTEXT set by
 ...
 * @returns {Integer} - If successful, returns the previous thread `DPI_AWARENESS_CONTEXT`. If
